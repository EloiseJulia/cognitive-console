...
def build_latex_table(arm_results: List[Dict[str, Any]], sesoi: float = SESOI) -> str:
    ...
    lines = [
        ...
        r"    SESOI = $\pm\delta = "
        + f"{sesoi:.2f}"
        + r"$ (symmetric to pre-registered superiority threshold).",
        r"    90\% bootstrap CI (item-cluster, $B=10{,}000$) used for TOST",
        r"    (two one-sided tests at $\alpha=0.05$); pre-reg Bonferroni 98.33\% CI shown for reference.",
        r"    Verdicts: \textsc{Harm} = CI$_{90}$ entirely below 0;",
        r"    \textsc{Equiv} = CI$_{90} \subset (-\delta, +\delta)$;",
        r"    \textsc{Under} = underpowered (CI spans 0 and/or $|$bound$|>\delta$).",
        r"    MDE\textsubscript{sup}: minimum effect detectable at 80\% power (Bonferroni superiority test).",
        r"  }",
    ...
    lines += [
        ...
        r"  \emph{Notes.}",
        r"  All numbers derived from frozen JSON artifacts (zero hand-coding).",
        r"  Pre-reg Bonferroni CI and pass/fail are authoritative (prereg-c2b-adjudication.md);",
        r"  TOST equivalence is post-hoc and exploratory (D-0056).",
