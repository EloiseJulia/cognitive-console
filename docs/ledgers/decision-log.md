...
## 2026-08-18 · D-0135 · Fold-gate Dispatch B powered skepticism re-measurement (additive, owner-delegated)
- **Owner-delegated (2026-08-18, owner out; blanket delegation to act on B/A within approved GPU budget):** materially pass the fold-gate for the additive powered skepticism re-measurement so the writing session can integrate it.
- **Evidence:** `e0016-powered-skepticism-b` on `feature/powered-remeasure-b@95b0905` (gen code `81c7e79`; provenance fixes `e42b3ef`). Frozen prereg `docs/specs/powered-tost-B-skepticism-prereg.md` (freeze `8941227`). Reuses E-0006 frozen α/direction (no re-selection); only TEST N scaled up to target MDE≤0.06; cap-to-ceiling on Llama at the fixed TruthfulQA item universe (N=797). 1.38 GPU-h.
- **Results (independently re-derived by audit):** CAA×Qwen mean −0.009 90%CI[−0.038,+0.020] **BOUNDED_EQUIVALENT**; ITI×Qwen −0.020 [−0.044,+0.004] **BOUNDED_EQUIVALENT**; CAA×Llama +0.028 [+0.001,+0.054] **UNDERPOWERED**; ITI×Llama +0.028 [+0.002,+0.054] **UNDERPOWERED**. MDEs 0.0571/0.0475/0.0510/0.0508. Coherence pass; parse 100% all cells; missingness 0.
- **Independent hostile audit:** `audit-powered-b` = CONDITIONALLY SOUND → SOUND after both provenance MAJORs closed (parse manifest committed; direction reconstruction caveat recorded). No BLOCKER. Report `reviews/2026-08-18-powered-skepticism-b-audit/audit-report.md`.
- **Scope (binding):** paper may state the **Qwen** skepticism no-pass **survives near δ=0.05 under powered additive re-measurement** (defeats the R1 "underpowered null" attack for skepticism-Qwen). **Llama** cells must be stated as **underpowered/indeterminate** (90% TOST upper bound slightly exceeds +0.05; item-universe-limited). Skepticism-axis only, single dataset (TruthfulQA), additive appendix; does NOT revise the frozen 0/12 headline or E-0005/E-0006/E-0011. Direction reuse is frozen-code reconstruction (no persisted vector), recorded as caveat.
- **Bookkeeping note:** main is at D-0134; latent-pc branch separately used D-0114 (pre-dates awareness of main's D-0112..D-0134) and must be renumbered at merge time to avoid decision-log collision.
- **Frozen?** valid_for_paper=true for the narrow additive skepticism scope; TEST-once, no re-selection.
