...
- 滑块每一档背后是一段 **冻结的、通用的单维风格预设**，只做语气/正式度位移，**绝不编码
  任务成功条件**；预设文本 **只存服务端，不回显给前端**。
- **构念效度（修 A）**：任务成功条件（vegan / <40 词 / friendly / 不用感叹号）= participant_goal，
  **只在 UI 展示、记录进导出，永不自动进模型输入**。两条件共享同一中性 `model_context`
  （不含成功条件）；滑块条件模型永远拿不到成功条件（天然暴露滑块局限），自写条件仅当参与者
  自己写入时才传达。
