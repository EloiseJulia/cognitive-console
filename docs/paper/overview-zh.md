...
>
> **最新进展（2026-08-05，本文档正文尚未逐节同步，以 `main.tex` + decision-log D-0074..D-0091 为准）：**
> 0. **D-0091 E-0016 pre-GPU audit：NOT SOUND。** 当前实现分支 `feature/e0016-ablation@c1c4d90` 有 4 BLOCKER（hook-bites 在 DEV generation 后才跑；只比较全局 max 而非逐 token 且不验证全层覆盖；HF real-data guard 可被改名本地 CSV 绕过；HF 默认仍是 DEV=4/TEST=8/K=2/layers1-3 的 smoke 配置）和 2 MAJOR（provenance 缺 seed/model revision/真实 dirty/config hash；测试未覆盖 actual-hook no-op/wrong/missing-layer、runner eligibility stop、immutable hash reject）。**不得 merge/freeze/GPU。** 新 Manager 必须先派修复、复审 SOUND；synthetic smoke PASS 不构成放行证据。
> 0. **D-0087..D-0089 最新进展：** 论文已把 F2 开放性前置到摘要/引言，把 evaluation contract 明确为首要方法学贡献，并以 appendix-level logit 诊断补强 READ≠CONTROL：refusal first-token mass 在 coherent β=1 被强推（均值 Δlogsumexp≈+13.9 nats），但 0/5 scored refusal，且 first-token proxy 会把普通/hedged answer 的 “I” 也计入；因此只能写成 representational push ≠ behavioral control，不能写成 handle works。最新 critic 给 **BORDERLINE / IUI 约 30–35%**，F2 已从 fatal 降为 MAJOR。Owner 决定继续做 E-0016 Arditi all-layer refusal-direction projection-ablation 正控：先跑 **Regime B（benign XSTest）**，DEV baseline false-refusal ≥0.25 才进入 TEST；若不足功效则停止，再由 owner 决定是否升级 harmful Regime A。E-0016 当前只完成设计+实现分支，仍在 pre-GPU hostile code audit；**没有冻结、没有 GPU run、没有结果，不得写入论文。**
