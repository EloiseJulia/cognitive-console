...
## 2026-08-18 · D-0114 · Owner-authorized fold-gate for E-0017f latent positive control (procedure-sensitivity, narrow scope)
- **Owner decision (via Manager, 2026-08-18):** authorize the three materially-verifiable gates for E-0017f so the writing session may fold it: (1) persist the independent hostile audit to the repo, (2) register E-0017f in experiment-registry, (3) flip artifact valid_for_paper false->true.
- **Evidence:** `E-0017f-phi3-official-stolfo-positive-control-stage2-test-once` on `feature/latent-positive-control@41b6f9d` (run code `78db8d0`). TEST held-out n=45, DEV/TEST overlap 0, frozen cell layer 28 x weight 80, official generation length 1024, frozen adjudicator (delta=0.05, item-cluster bootstrap B=10000, Bonferroni 98.33%, coherence gate). primary steer-baseline +0.2222 CI [0.0889, 0.3778] (clears 0), coherence 0.1023<=0.1116 PASS, secondary steer-prompt -0.4444 CI [-0.6444,-0.2444]. Artifact SHA-256 (post valid-flip) `293ce23af5db236d841c536d37f3d7e8d180243e11a795c51249fa22e225b599`.
- **Independent hostile audit:** `audit-latentpc` = PASS SOUND, no numerical BLOCKER; re-derived numbers exactly; robust to unique-key / whole-word / truncation-excluded sensitivity. Report: `reviews/2026-08-18-e0017f-latent-pc-audit/audit-report.md`.
- **Scope (binding):** procedure-sensitivity positive control ONLY (Phi-3-mini + official Stolfo keyword-existence; a DIFFERENT model/task than the headline Qwen/Llama x 3 axes). Shows the frozen adjudication procedure can certify a genuine latent control. Does NOT validate headline-axis assay sensitivity, does NOT make the three console axes controllable, does NOT revise the frozen 0/12 grid or E-0005/E-0006/E-0011.
- **Required paper disclosures (audit MAJORs, binding on the writing session):** full Qwen->Gemma(gated, not run)->Phi-3 model path (fallback not pre-registered before the Qwen failure); report 45 task rows / 20 unique source keys with unique-key sensitivity; official verifier is regex substring (whole-word sensitivity also passes). §7.2 L584 "no latent positive control passes" must be replaced with the narrow-scope wording; must NOT be conflated with the B1 prompt-level control at L33/L100/L636.
- **Frozen?** valid_for_paper=true for the narrow procedure-sensitivity claim; TEST-once, no re-selection. Does not authorize any headline-axis or cross-model latent controllability claim.
