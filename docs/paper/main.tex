...
\begin{abstract}
Interfaces increasingly place controls beside named latent properties, implying that moving the control will reliably change the corresponding behavior. Readability and output movement do not establish that promise when prompting already offers a behavioral alternative. We contribute a comparator-bound evaluation contract: an active latent control must earn its affordance by improving a declared behavioral outcome over a bounded, DEV-selected prompt comparator, under an uncertainty rule, a point-estimate floor, and a coherence gate. Representational-readability (READ) evidence, the comparator, a calibration warning, and an evidence tier record what the interface may claim. Applied in a prompt-versus-latent substitution design across Qwen and Llama, CAA and ITI, and three behavioral axes, the contract qualifies none of the twelve model--method--axis tests and resolves that shared outcome into distinct, actionable profiles: skepticism is bounded-equivalent to the best prompt within the \(\pm0.05\) margin in the pre-registered refined Qwen--CAA cell (its achievable resolution slightly exceeds the floor) with the remaining skepticism cells resolution-limited, deliberation accuracy is floored by the short generation budget and is being re-measured, and all four uncertainty contrasts are negative under the frozen scorer (one rechecked cell missingness-limited with an unidentified sign, three unrechecked). A predeclared positive control confirms the rule can qualify and discriminate a control, not only withhold one, so this outcome reflects the evidence rather than an unsatisfiable gate. Because no latent positive control passed, the empirical contribution is a worked procedural boundary rather than a validated assay or a general steering verdict, and the interface mapping is a designer-facing procedure whose user outcomes a planned study will probe (comprehensibility, not benefit).
\end{abstract}
