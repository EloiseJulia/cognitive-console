...
{}% FL-043
{}% FL-047
The contract preserves these differences as structured evidence and blocking reasons instead of converting 0/12 into a universal null.

The contract maps computational results to structured evidence and then to interface eligibility. READ support makes a representation a \textbf{read-only diagnostic candidate within this evidence tier}. An active control passes the computational gate within this evidence tier only when READ is supported and the reported TRANSFER rule passes; otherwise the record keeps the blocking reason visible and the active control is withheld. In the worked Qwen--CAA uncertainty example, the slider is paused while the read-only diagnostic remains a candidate. {}% OI-014
...
\item
  \textbf{A scoped complete 2×2 worked boundary.} The CAA/ITI by Qwen/Llama grid demonstrates how one procedural outcome retains distinct blocking reasons and next evaluations, with a record-specific interface action illustrated only for Qwen--CAA uncertainty.
\end{enumerate}
...
{}% FL-031
The record therefore preserves an information need rather than assigning a scientific state.
...
For this record, the exploratory READ result makes uncertainty a \textbf{read-only diagnostic candidate within this evidence tier}. The TRANSFER result is no-pass relative to the comparator, and the all-generation contrast is missingness-limited. {}% C-005
{}% C-007
