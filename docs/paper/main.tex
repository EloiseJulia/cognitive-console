...
\title{Convenient or Warranted? Evaluating Latent Sliders for Everyday Users and Experts}
...
A single prompt comparator hides the population question. We therefore use two, each standing for a different user's realistic alternative: a strong best-of-set prompt, selected on DEV from a preregistered set of sixteen candidates, represents the expert's ceiling; an average prompt over that same set represents what a non-expert is likely to write. {}% FL-008
{}% FL-009
...
\subsection{Against the Expert's Ceiling, No Latent Arm Wins}\label{against-the-experts-ceiling-no-latent-arm-wins}

We first evaluate the expert's standard: does a latent intervention earn incremental behavioral advantage over a strong prompt? Across Qwen and Llama, CAA and ITI, and three behavioral axes, none of the twelve tests beats the DEV-selected best-of-set prompt under the reported rule. A stronger, DEV-optimized latent method fails the same way, and the direct steer-versus-baseline contrast is near zero---so the result is not an artifact of one weak method. This grounds the expert's skepticism: here, moving the slider buys no warranted quality gain over an instruction. {}% FL-003
{}% FL-042
{}% C-003
The everyday user's standard turns instead on the average prompt and on convenience; the average-prompt comparator arm and a planned user study address it, and its slot is marked pending rather than pre-judged. Within the expert comparison, the twelve no-passes still differ in kind---bounded equivalence for the refined skepticism cell, a generation-length--floored measurement for deliberation that is being repeated, and a comparator-specific calibration warning for uncertainty---which matters for what to measure next, not for whether the expert's bar was cleared.
...
\paragraph{What a designer gains, for each user.} The record's user is the interface designer or team deciding whether to release a control---and it separates the two populations the decision serves. On the expert's standard, the record already answers: no tested control earns incremental behavioral advantage over a strong prompt, so none should be shipped as an expert-grade quality control on this evidence. On the everyday user's standard---low effort, an average prompt, convenience---the record withholds judgment until the average-prompt comparator and a user study report, and says so explicitly rather than borrowing the expert result. In both cases it still blocks the two substitutions it is built to prevent: promoting a control on READ or on output movement alone.
...
{}% FL-045
The comparator changes what the same observed gap means for the interface. Choosing a comparator is therefore choosing a population: a strong prompt asks the expert's question, an average prompt asks the everyday user's, and a control can clear one bar while missing the other.
