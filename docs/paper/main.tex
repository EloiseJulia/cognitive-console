...
\paragraph{What a designer gains today.} The record's user is the interface designer or team deciding whether to release a control. Even when every candidate is a no-pass, the record is immediately actionable: it names which control not to ship now, which representation may still be exposed read-only, which axis is resolution-limited near the floor and can be re-measured directly (skepticism), and which needs measurement repair before any claim (uncertainty's missingness)---while blocking the two substitutions it is built to prevent, promoting a control on READ or on output movement alone.
