    ...
    for model_key in sorted({cell.model_key for cell in cells}):
        ...
        for cell in model_cells:
            if cell.method == "caa":
                ...
                direction_meta = {"method": "caa", "sigma": 1.0}
            elif cell.method == "iti":
                pairs = c1.load_axis_pairs(cell.axis)
                split = c1.make_split(
                    list(pairs.pos.keys()), n_extraction=N_EXTRACTION, seed=SEED
                )
                pos_texts = [pairs.pos[p] for p in split.extraction_ids]
                neg_texts = [pairs.neg[p] for p in split.extraction_ids]
                iti = extract_iti(
                    provider,
                    axis=cell.axis,
                    pos_texts=pos_texts,
                ...
                direction_meta = {
                    ...
                    "selection": "fixed_layer_from_cell_source",
                }
            else:
                raise ValueError(f"unsupported cell method: {cell.method!r}")
...
def _sampler_factory(
    ...
    *,
    ...
    factory = base.hf_sampler_factory(
        ...
        batch_size,
        collector,
        alpha_scale_by_axis={cell.axis: float(alpha_scale)},
        model_revision=cell.model_revision,
    )
...
def _dev(args) -> int:
    ...
    for cell in CELLS:
        ...
        alpha_scale = (
            spec_meta.get("directions", {}).get(cell.cell_id, {}).get("sigma", 1.0)
            if cell.method == "iti"
            else 1.0
        )
