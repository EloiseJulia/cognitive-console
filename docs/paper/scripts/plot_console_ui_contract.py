...
def _line_ops(x: float, y: float, text: str, size: int = 8) -> str:
    return f"BT /F1 {size} Tf {x:.1f} {y:.1f} Td ({_esc(text)}) Tj ET\n"
...
def _content_stream(payload: dict) -> str:
    ...
    ops.append(_line_ops(56, 468, "Method-strength robustness marker (E-0009 PSR)", 11))
    psr_bits = [
        f"{row['label']}: pass={row['passed']} Δ={_fmt(row['delta'])} CI=[{_fmt(row['ci_lo'])},{_fmt(row['ci_hi'])}]"
        for row in psr["rows"]
    ]
    ops.append(_line_ops(56, 450, f"{psr['summary']} Verdict={psr['verdict']}.", 8))
    ops.append(_line_ops(56, 436, " | ".join(psr_bits), 7))
    ops.append(_line_ops(44, 396, "Lineage: E-0003/E-0005/E-0006/E-0009/E-0010; generated by docs/paper/scripts/plot_console_ui_contract.py", 7))
    return "".join(ops)
