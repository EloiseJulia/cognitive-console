...
>
> **最新进展（2026-08-05，本文档正文尚未逐节同步，以 `main.tex` + decision-log D-0074..D-0093 为准）：**
> 0. **D-0092/D-0093 E-0016 当前活动状态：** D-0091 的 scoped harness blockers 已由独立 delta audit 关闭，audited harness 已合并为 `4513820a679408b071984b8500cc9c5636255a49`。`c36a438977ae51be5172587e900811508fe77468` 仅是包含首次冻结文档的 **protocol freeze candidate**；当前仍在 targeted protocol audit，修复合并后的最终 run commit 尚待 Manager 登记，owner GPU budget 也未批准。**无 GPU run、无 result、不得写入论文。**
> 0. **历史状态（D-0091，已被 D-0092/D-0093 supersede）：** 当时 E-0016 pre-GPU audit 为 NOT SOUND，`feature/e0016-ablation@c1c4d90` 有 4 BLOCKER（hook-bites 在 DEV generation 后才跑；只比较全局 max 而非逐 token 且不验证全层覆盖；HF real-data guard 可被改名本地 CSV 绕过；HF 默认仍是 DEV=4/TEST=8/K=2/layers1-3 的 smoke 配置）和 2 MAJOR（provenance 缺 seed/model revision/真实 dirty/config hash；测试未覆盖 actual-hook no-op/wrong/missing-layer、runner eligibility stop、immutable hash reject）。该段仅保留历史审计证据，不代表当前 harness 状态；synthetic smoke PASS 从未构成放行证据。
> 0. **D-0087..D-0089 研究背景：** 论文已把 F2 开放性前置到摘要/引言，把 evaluation contract 明确为首要方法学贡献，并以 appendix-level logit 诊断补强 READ≠CONTROL：refusal first-token mass 在 coherent β=1 被强推（均值 Δlogsumexp≈+13.9 nats），但 0/5 scored refusal，且 first-token proxy 会把普通/hedged answer 的 “I” 也计入；因此只能写成 representational push ≠ behavioral control，不能写成 handle works。最新 critic 给 **BORDERLINE / IUI 约 30–35%**，F2 已从 fatal 降为 MAJOR。Owner 决定继续做 E-0016 Arditi all-layer refusal-direction projection-ablation 正控：先跑 **Regime B（benign XSTest）**，DEV baseline false-refusal ≥0.25 才进入 TEST；若不足功效则停止，再由 owner 决定是否升级 harmful Regime A。该段仅说明立项依据；E-0016 当前活动 gate 以紧邻上方 D-0092/D-0093 状态为准，仍**没有 GPU run、没有结果，不得写入论文。**
