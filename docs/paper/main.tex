...
\begin{abstract}
Interfaces increasingly place sliders beside named latent properties. For an everyday user a slider is convenient---nudging it is far easier than composing the right instruction---while a proficient user asks a different question: does moving it actually improve the named behavior beyond what an instruction already achieves? Convenience and warranted incremental control are two standards, and ``is the slider good?'' depends on who is asking and against which alternative. We make that alternative explicit by operationalizing each population's realistic option as a prompt comparator: a strong, DEV-selected best-of-set prompt for the proficient user's ceiling, and an average prompt for what a non-expert is likely to write. A latent intervention is judged against a declared behavioral outcome under an uncertainty rule, a 0.05 point-estimate floor, and a coherence gate; READ evidence, the comparator, a calibration warning, and an evidence tier record what the result licenses. Applied to CAA and ITI on Qwen and Llama across three behavioral axes in a substitution design, naive---and a stronger DEV-optimized---latent steering beats the strong prompt on none of the twelve model--method--axis tests, and a direct steer-versus-baseline contrast is near zero; a predeclared positive control shows the rule can also qualify a control rather than only withhold one. No latent positive control passes, so we report a worked boundary rather than a general steering verdict: the proficient user's standard---warranted quality gain over a strong prompt---is not met here, while the everyday user's standard, which turns on convenience and on the average-prompt comparator, is addressed by a planned user study and a comparator arm rather than settled by these results.
\end{abstract}
...
None of the twelve model--method--axis tests qualifies under the reported rule---the procedure doing its job: no candidate earned an active control, so each is withheld rather than promoted on legibility or output movement alone. That common computational result does not imply one common scientific conclusion or interface action. In pre-registered refinements, skepticism is bounded-equivalent in the Qwen--CAA cell (at a resolution slightly above the floor), while the deliberation measurement is floored by the short generation budget and is being repeated; uncertainty is negative relative to the comparator under the frozen scorer, with a targeted Qwen--CAA recheck whose complete-case estimate remains negative but whose all-generation missingness bounds cross zero. {}% FL-003
{}% FL-004
{}% FL-041
{}% FL-043
{}% FL-047
