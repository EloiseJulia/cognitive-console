...
As a prospective check that the qualification rule can \emph{qualify} a control and not only withhold one, we ran the unchanged adjudicator on a predeclared, format-following prompt effect over a pool disjoint from the tested items. It accepted a non-degenerate instruction advantage (\(\Delta=0.60\), 98.33\% item-cluster bootstrap CI \([0.45,0.75]\)) and rejected both an exact null and an independently resampled noisy null (\(\Delta=0.01\), CI \([-0.03,+0.05]\)). The qualification rule is therefore mechanically satisfiable and discriminative---the all-fail grid is not an artifact of an unsatisfiable gate---but this validates basic decision-rule behavior, not latent-steering efficacy or near-threshold calibration. {}% experiment_ids: prospective-pc-noisy (feature/positive-control-noisy@364ee43) + strong-negative (feature/positive-control-strong-negative@5d6dbe1); PENDING lineage reconciliation onto the writing branch
