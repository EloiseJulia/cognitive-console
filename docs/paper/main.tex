...
The contract maps computational results to structured evidence and then to interface eligibility. READ support makes a representation a \textbf{read-only diagnostic candidate} within its evidence tier. An active control passes the computational gate within that tier only when READ is supported and the reported TRANSFER rule passes; otherwise the record keeps the blocking reason visible and the active control is withheld. In the worked Qwen--CAA uncertainty example, the slider is paused while the read-only diagnostic remains a candidate. {}% OI-014
...
Equation \ref{eq:qualification-rule} requires an interval excluding zero and a point estimate at least \(\delta\); it does \textbf{not} require the interval's lower bound to exceed \(\delta\). This operationalizes a positive-advantage release criterion---an interval excluding zero with a point estimate that clears \(\delta\)---without demanding the largest defensible effect; a stricter release can raise \(\delta\) or require the lower bound to clear it. The contract is the requirement---a declared comparator, outcome, uncertainty rule, floor, and coherence gate---while the specific values (\(\delta=0.05\), the coherence ratio, and the two-axis aggregation) are this application's settings rather than part of the portable procedure. The protocol and executable adjudicator were frozen on 2026-07-23. The degradation score is one minus the distinct trigram ratio for each generation; scores are averaged within the steering and unsteered cells, and the steering mean must satisfy \(g_a^S\le1.5g_a^0+0.02\). The implementation adds \(10^{-12}\) only as floating-point comparison tolerance.
...
The record is designed to guard against three substitutions in interface reasoning: READ for TRANSFER, absolute output change for comparison against the declared alternative, and evidence from one tier for another. It reports the computational result, preserves the structured evidence or blocking reason, and then specifies the record-specific interface action. This mapping is a proposed design procedure rather than a validated user outcome.
