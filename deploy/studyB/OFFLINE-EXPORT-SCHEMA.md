...
| `tasks` | list | One entry per paired main task (see below). |
| `convenience` | object | Short TLX + Likert + willingness (see below). |
| `reliance` | object or `null` | Optional confidence ratings; `null` if skipped. |
...
| Key | Type | Notes |
|---|---|---|
| `tlx_mental` | int | Short TLX mental demand, 1–7. |
| `tlx_effort` | int | Short TLX effort, 1–7. |
| `tlx_frustration` | int | Short TLX frustration, 1–7. |
...
## `reliance` (optional; `null` if skipped)

| Key | Type | Notes |
|---|---|---|
| `confidence_slider` | int or `null` | Confidence the slider met the goals, 1–5. |
| `confidence_own_prompt` | int or `null` | Confidence own prompt met the goals, 1–5. |
