...
Equation \ref{eq:qualification-rule} requires an interval excluding zero and a point estimate at least \(\delta\); it does \textbf{not} require the interval's lower bound to exceed \(\delta\). This operationalizes a positive-advantage release criterion---an interval excluding zero with a point estimate that clears \(\delta\)---without demanding the largest defensible effect; a stricter release can raise \(\delta\) or require the lower bound to clear it. We set a release bar rather than a scientific one: \(\delta=0.05\) is a minimal behavioral advantage a designer would demand before adding a control, the coherence gate blocks fluency collapse, and because a false active control is costlier than a withheld one, the rule favors withholding under uncertainty. The contract is the requirement---a declared comparator, outcome, uncertainty rule, floor, and coherence gate---while the specific values (\(\delta=0.05\), the coherence ratio, and the two-axis aggregation) are this application's settings rather than part of the portable procedure. The protocol and executable adjudicator were frozen on 2026-07-23. The degradation score is one minus the distinct trigram ratio for each generation; scores are averaged within the steering and unsteered cells, and the steering mean must satisfy \(g_a^S\le1.5g_a^0+0.02\). The implementation adds \(10^{-12}\) only as floating-point comparison tolerance.
