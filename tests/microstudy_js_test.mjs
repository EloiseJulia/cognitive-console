...
import {
  createEmptyTrial, csvEscape, submitQ1, submitQ2,
} from "../src/cognitive_console/microstudy/static/study_core.mjs";

const slot = {
  slot_index: 1, condition: "Contract", item: "MS-P1-X", pattern: "P1",
  content_set: "X", block: 1, position: 1,
};
const item = {source_status: "synthetic_rule_case", source_note: "simulated", hypothetical: true};
const trial = createEmptyTrial(slot, item, "v1");
assert.equal(trial.planned, true);
assert.equal(trial.presented, false);
assert.throws(() => submitQ2(trial, "A", "A", 10, 0), /Q1 must be locked/);
submitQ1(trial, "Q1_A", "Q1_A", 12.4);
assert.equal(trial.q1_submitted, true);
assert.equal(trial.rt_q1_ms, 12);
assert.throws(() => submitQ1(trial, "Q1_B", "Q1_B", 3), /already locked/);
assert.equal(trial.q1, "Q1_A");
