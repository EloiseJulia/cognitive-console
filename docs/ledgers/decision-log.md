...
## 2026-08-19 · D-0136 · Fold-gate Dispatch A composition (well-powered null, additive, owner-delegated)
- **Owner-delegated (2026-08-18/19, owner authorized both B/A within approved GPU budget and delegated fold-gate decisions):** materially pass the fold-gate for the composition/augmentation additive result so the writing session can integrate it.
- **Evidence:** `composition-a-qwen-caa-20260818-0001` on `feature/composition-a@f3494a0` (audit-major fixes `d75ba65`). Frozen prereg `docs/specs/c2b-composition-augmentation-prereg.md` (freeze `3b2defc`). Composition estimand paired (prompt+steer) − (prompt alone); ordinary + strong baselines; 3 axes; CAA x Qwen. composition-DEV α re-selected then frozen before TEST; DEV/TEST zero overlap. 8.4 GPU-h.
- **Results (independently re-derived by audit, prereg-seed recompute):** all 6 cells **NO_INCREMENT_DEMONSTRATED**, well-powered (MDE 0.0178–0.0231, all ≤ target 0.06), all 98.33% CI include 0. ordinary: delib +0.0045[−0.0085,+0.0175]; skep +0.0055[−0.0090,+0.0205]; uncert −0.0055[−0.0201,+0.0086]. strong: delib −0.0105[−0.0255,+0.0050]; skep −0.0005[−0.0175,+0.0165]; uncert +0.0034[−0.0098,+0.0167]. coherence pass; parse 0.99+; uncertainty missingness bounds do not support gain.
- **Independent hostile audit:** `audit-composition-a` = CONDITIONALLY SOUND → SOUND after both MAJORs closed (prereg-seed recompute; uncertainty missingness bounds). Report `reviews/2026-08-19-composition-a-audit/audit-report.md`.
- **Scope (binding):** paper may state that **even in the composition regime (steering added on top of a user instruction), CAA×Qwen provides no detectable incremental behavioral gain over the prompt alone on any of the three axes** — a well-powered null answering the "substitution-only ≠ real slider" BLOCKER. NOT an equivalence proof; NOT a global "steering never helps" claim. Single method (CAA), single model (Qwen); **ITI/Llama untested**; ordinary prompt is a plain-instruction everyday-user proxy (human-anchor gap). Additive; does NOT revise the frozen 0/12 headline or E-0005/E-0006/E-0011. Direction reuse = frozen-code reconstruction (no persisted vector, caveat).
- **Frozen?** valid_for_paper=true for the narrow additive composition null scope; TEST-once, no re-selection.

## 2026-08-19 · D-0137 · Fold-gate composition ITI×Qwen shard (method-generalization, owner-delegated)
- **Owner-delegated (2026-08-19, owner approved ITI×Qwen method-generalization shard):** materially pass the fold-gate for the ITI composition shard.
- **Evidence:** `composition-a-qwen-iti-20260818-0001` on `feature/composition-a@a569407`. Same frozen prereg `c2b-composition-augmentation` (freeze `3b2defc`) as CAA shard; only method=ITI (with σ scale). Same frozen TEST items/prompts, bootstrap_seed=20260819. 10.5 GPU-h.
- **Results (independently re-derived by audit, bit-match):** 5/6 cells valid, all NON-POSITIVE. ordinary: delib −0.0090[−0.028,+0.010] NO_INCREMENT; skep −0.0175[−0.043,+0.007] NO_INCREMENT; **uncert −0.0441 INVALID (parse gate fail, 47/400 missing, diagnostic-only NOT harm)**. strong: **delib −0.0255[−0.046,−0.007] REAL small negative (coherence PASS)**; skep −0.0250[−0.054,+0.004] NO_INCREMENT; uncert −0.0123[−0.028,+0.004] NO_INCREMENT.
- **Independent hostile audit:** `audit-composition-iti` = SCIENCE SOUND (all 6 cells bit-match). Report `reviews/2026-08-19-composition-iti-audit/audit-report.md`.
- **Scope (binding):** combined with CAA shard (E-COMPOSITION-A), **Qwen composition shows a ROBUST absence of positive increment across BOTH methods (CAA and ITI)**. ITI additionally trends slightly negative; **strong-deliberation is a real small decrement** (steering on top of a strong prompt slightly degrades) and must be reported as such, not as null. **ordinary-uncertainty is INVALID (parse gate)** — diagnostic-only, NOT harm evidence. Single model (Qwen); Llama untested; ordinary prompt = plain-instruction proxy (human-anchor gap). Additive; does NOT revise frozen 0/12 or E-0005/E-0006/E-0011. Direction = frozen-code reconstruction (caveat).
- **Frozen?** valid_for_paper=true for the narrow additive composition scope; TEST-once, no re-selection.
