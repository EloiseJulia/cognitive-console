...
## 2026-08-12 · E-0016 safe-proxy refusal-ablation positive control terminated: valid mechanism, no behavioral headroom
- Qwen2.5-7B-Instruct passed direction derivation (source layer `8`, separation approximately `4.0` versus the `0.8` floor, unit-norm direction) but its benign XSTest-safe DEV baseline false-refusal rate was only `0.0033` (`0.33%`). The frozen `>=0.25` eligibility screen correctly returned `INVALID_REGIME_B_UNDERPOWERED` and stopped before ablation selection or TEST.
- Llama-3-8B-Instruct likewise showed strong harmful-vs-harmless representational separation at every candidate layer (for example, layer `20` separation `7.37`) while the derived refusal direction was barely engaged by the safe evaluation prompts: extraction-layer max projection fraction was about `0.021`, mean about `0.009`, and aggregate about `0.0025`; absolute projection was only about `0.30` against residual-stream norms spanning roughly `46–390`.
- This is not a broken hook or degenerate direction. Well-aligned 7–8B instruct models rarely refuse these safe prompts, so the safe set contains almost no falsely-refused items that activate the refusal direction strongly enough for ablation to reduce refusal. High contrast separation can therefore coexist with near-vacuous intervention on the evaluation distribution.
- **Conclusion:** E-0016 is mechanistically sound but ill-posed as a behavioral positive control on this safe proxy/model regime. Its fail-closed guards correctly refused to certify a vacuous intervention and remain unchanged. E-0016 is terminated as an honest negative; the project will use ITI × TruthfulQA as the positive-control route instead. No harmful generation, Regime A, TEST-informed tuning, raw harmful-text commit, or guard/pass-rule weakening occurred.
