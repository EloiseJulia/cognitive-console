...
Together, these contributions turn a familiar interface question into a versioned decision record: evidence first, affordance second. The release rule is scoped to substitution-type controls---a latent affordance offered \emph{in place of} a prompt; a control that composes on top of a user's instruction is out of scope and would need composition evidence this evaluation does not provide.
...
Across Qwen and Llama, CAA and ITI, and three behavioral axes, none of the twelve tests passed the reported TRANSFER rule; consequently no active control became eligible. This is the qualification working as designed rather than a failed experiment: every procedural candidate is withheld until it earns the affordance. {}% FL-003
{}% FL-042
{}% C-003
The contract turns that aggregate into three structured evidence profiles: bounded equivalence for the refined skepticism cell, a floored generation-length measurement for deliberation that is being repeated, and a comparator-specific calibration warning for uncertainty. These profiles guide different next evaluations, rather than collapsing all twelve tests into one universal null.
...
The predeclared positive-control check (Section~\ref{the-contract-separates-three-evidence-profiles}) shows the adjudicator is mechanically satisfiable and discriminative, so the all-fail grid is not an artifact of an unsatisfiable gate; that check bears on decision-rule behavior, not latent-steering efficacy or near-threshold calibration. {}% experiment_ids: prospective-pc-noisy (feature/positive-control-noisy@364ee43) + strong-negative (feature/positive-control-strong-negative@5d6dbe1); result JSONs on writing branch
