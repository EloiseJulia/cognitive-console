**使用前：**先完成“Research Layer”的研究宪章、权限与预算配置，再执行后半部分的 Engineering Workflow。目标是最大化达到目标顶会标准的概率，但不得以选择性汇报、测试集泄漏、隐瞒负结果、伪造引用、夸大主张或无限消耗资源为代价。
...
# Part II · Implementation & Artifact Production Subsystem

以下保留并复用原文的 Spec-Driven Development、worktree、PR、独立代码审计与真实产物验证流程。它负责“如何可靠地产出实现与 Artifact”，但其上游研究方向、实验授权、证据有效性和投稿主张必须服从 Part I。
