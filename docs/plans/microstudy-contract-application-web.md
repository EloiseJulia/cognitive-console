...
- **Status:** implementation complete; pending fresh hostile audit
...
The loopback website consumes the two JSON files directly. It caches parsed
records in memory but does not embed another copy of stimuli, templates, answers,
tutorial/practice, sequences, diagnostic, ease options, or export fields. Q1 keys
are generated by the router and checked against JSON expectations; Q2 keys come
...
- Rebuild keys, completion, eligibility, duplicate exclusions, CCA, missingness,
  bootstrap, and sign flips from raw exports.
- Provide a DRAFT assumption-sensitivity MDE simulation before protocol freeze.
...
`materials_implemented_pending_hostile_audit_no_human_data`; there is no human run,
