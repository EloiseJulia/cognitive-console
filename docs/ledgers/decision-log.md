...
## 2026-07-31 · D-0063 · E-0012b model-gen re-run audited NOT-SOUND (broken APE comparator); both E-0012 runs unsound as positive; §5 escalation to owner
- Independent hostile audit (`reviews/2026-07-31-e0012-modelgen-audit/audit-report.md`) verdict = **NOT-SOUND**. Two BLOCKERs: (1) frozen harness `generate_candidates_real` ran the APE generation GREEDILY, ignoring the frozen §5-B mandate temperature=0.9/top_p=0.9/seed=42/do_sample — a freeze-integrity bug (frozen code ≠ frozen spec; see failure-log 2026-07-31); (2) the 50 candidates were not persisted and the winner (id=45) is byte-identical to authored CAL-09 → comparator was greedy + heavily authored-fallback → unauditable. ⇒ the TRANSFER→LOCAL flip (APE k5 0.827→0.534) is almost certainly an artifact of the broken generator, not a real positive.
- **What the audit CONFIRMED solid:** DEV/TEST split 0-overlap, synthetic_proxy=false on all 22,311 raw pairs, N-01 guard active, Stage1 held-out CIs independently reproduced (button has a REAL calibration effect vs the stored comparator: PROBE δ=0.144 CI[0.086,0.198]), kill-rule/verdict arithmetic correct, Stage0 105/105 + Bonferroni handled. The button's behavioral effect is genuine; only the "beats a fair prompt baseline" (VERIFIED-CONTROL) claim is unsound.
- **MAJOR-1 (stands regardless of any re-run):** in the synthetic run a competent human-authored prompt reached DEV k5=0.827 ≥ button 0.697 → the button is NOT shown superior to good prompting. A correctly-implemented stochastic APE would very likely rediscover a strong prompt near 0.827 → most probable honest outcome of any fixed re-run is a REVERSION TO TRANSFER (the negative we already hold).
- **Manager governance:** both E-0012 runs are now unsound as a headline positive (synthetic=wrong comparator path F-01; model-gen=broken generator BLOCKER-1/2). The button's real-but-not-superior calibration effect still supports the paper's negative/boundary thesis (legible axis is prompt-reachable, not a verified control). **§5 escalation:** owner to choose (A) fix BLOCKER-1/2 + MAJOR-2 + re-run once more (another ~5-6h GPU, most-likely-TRANSFER), or (B) stop and report E-0012 as an honest inconclusive/negative boundary point, keeping the audited C1/C2 negative as the headline (no GPU). Manager recommendation: **B** (marginal value of chasing a clean positive is low; the honest thesis is stable either way). Paper unchanged; no positive claim shipped.
